...
**Answer:** Hibernate provides multiple ways to log or display the SQL queries it executes against the database.
This feature is extremely useful for debugging and performance tuning, as it allows developers to see the actual SQL statements generated from their entity operations.

1. Using hibernate.show_sql Property (Most Common Way)

You can enable SQL query logging directly in the hibernate.cfg.xml file by setting the following property:
...
- hibernate.show_sql → Displays the generated SQL queries on the console.

- hibernate.format_sql → Formats the SQL output for better readability.

2. Using Logging Framework (Recommended for Production)

Instead of printing SQL directly to the console, you can configure a logging framework (like Log4j, SLF4J, or Logback) to log the SQL queries.
...
# Enable Hibernate parameter value logging (optional)
log4j.logger.org.hibernate.type.descriptor.sql.BasicBinder=TRACE
```

**Explanation:**

- org.hibernate.SQL → Logs all SQL statements generated by Hibernate.

- org.hibernate.type.descriptor.sql.BasicBinder → Logs the actual parameter values passed to SQL queries.

This approach gives more control over where and how SQL logs are stored (e.g., file, console, etc.) and is preferred in real-world applications.

3. Programmatic Configuration (Optional Approach)

You can enable SQL logging programmatically within your Java code:
...
This method is useful when you configure Hibernate without using hibernate.cfg.xml.

4. Enable Detailed SQL Logging with Parameters (For Debugging)

If you want to log both SQL statements and their bound parameters, use these logging categories in log4j.properties or logback.xml:

```properties
# Show SQL queries
log4j.logger.org.hibernate.SQL=DEBUG

# Show parameters (bind values)
...
```pgsql
Hibernate: insert into student (name, course, id) values (?, ?, ?)
binding parameter [1] as [VARCHAR] - Rahul
binding parameter [2] as [VARCHAR] - Computer Science
